                    ...
                    <div className="space-y-3">
                        {grammarPoint.examples.map((example, index) => (
                            <div key={index} className="bg-slate-50 p-4 rounded-lg flex items-center gap-4">
                                ...
                                <button
                                    onClick={() => handlePlay(example.thai)}
                                    ...
                                    aria-label={`例文「${example.thai}」を再生`}
                                >
                                    {isSpeaking && speakingSentence === example.thai ? <LoadingSpinner className="h-5 w-5" /> : <PlayIcon className="h-6 w-6" />}
                                </button>
                            </div>
                        ))}
                    </div>
